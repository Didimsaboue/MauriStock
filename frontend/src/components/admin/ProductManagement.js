  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        alert(response.data?.message || 'Produit enregistré avec succès');
        fetchProducts();
        resetForm();
      } else {
      ...
      alert(error.response?.data?.message || 'Une erreur est survenue lors de l\'enregistrement du produit');
    }
  };    
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    {editingProduct && editingProduct.picture && !formData.picture && (
                      <div className="mt-2">
                        <img
                          src={`http://localhost:5000${editingProduct.picture}`}
                          alt="Image actuelle"
                          className="h-20 w-20 object-cover rounded"
                        />
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-4xl">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                <img
                  src={`http://localhost:5000${viewProduct.picture}`}
                  alt={viewProduct.nom}
